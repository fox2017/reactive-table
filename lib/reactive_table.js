...
var setup = function () {
    ...
    //show pagination buttons number;
    context.paginationBtn= this.data.paginationBtn || this.data.settings.paginationBtn || 1;
...
Template.reactiveTable.helpers({
    ...
    'getCurrentPage' : function (obj) {
        return 1 + (obj||this).currentPage.get();
    },
    ...
    'getPageInfo':function () {
        ...
        return {
            ...
            end:(currentPage+1)*rowsPerPage,
            thePage:currentPage
        }
    },
    'getPageBtn':function () {
        var _tmp=[],btns = this.paginationBtn;
        var pageCount = getPageCount.call(this);
        var currentPage = this.currentPage.get()+1;
        var qs=Math.floor(btns/2),startBtn=currentPage<=btns?1:(currentPage-qs);
        var pages=0;

        btns=Math.min(btns,pageCount);

        if(currentPage>btns)_tmp=_tmp.concat([
            {
                n:false,
                i:this.useFontAwesome,
                l:'>',
                v:1,
                c:'fa-fast-backward'
            },
        ...
        for(var i=0;i<btns;i++){
            (startBtn+i)<=pageCount&&_tmp.push({
                n:true,
                l:startBtn+i,
                v:startBtn+i,
                c:currentPage===startBtn+i?'active':''
            });
            pages=i;
        }
        if((startBtn+pages)<pageCount)_tmp=_tmp.concat([
            {
                n:false,
                i:this.useFontAwesome,
                l:'>',
...
Template.reactiveTable.events({
    ...
    'click .reactive-table-options .rows-per-page .dropdown-menu a':function (event) {
        var rowsPerPage = Math.max(~~$(event.target).text(), 1);
        var template = Template.instance();
    ...
    'change .reactive-table-options .rows-per-page input': function (event) {
    ...
    'change .reactive-table-navigation .page-info input': function (event,tpl) {
    ...
    'click .reactive-table-navigation .page-number .btn-current':function (event,tpl) {
        var template = Template.instance();
        template.context.currentPage.set(this.v-1);
        template.updateHandle(template.context);
    }
});

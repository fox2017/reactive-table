...
ReactiveTable.publishAggregate = function (name,pipOrFunction,collectionOrFunction, selectorOrFunction, settings,option) {
    Meteor.publish("reactive-table-" + name, function (publicationId, filters, fields, options, rowsPerPage) {
        check(publicationId, String);
        check(filters, [Match.OneOf(String, Object, Mongo.ObjectID)]);
        check(fields, [[String]]);
        ...
        var selector;
        var pipLine;
        var pipOptions=_.extend({observeSelector: {},observeOptions: {}},option);



        if (_.isFunction(collectionOrFunction)) {
            collection = collectionOrFunction.call(this,filters);
        } else {
            collection = collectionOrFunction;
        }
        ...
        // if (_.isFunction(selectorOrFunction)) {
        //     selector = selectorOrFunction.call(this,filters);
        // } else {
        //     selector = selectorOrFunction;
        // }
        var self = this;
        ...
        if (_.isFunction(pipOrFunction)) {
            pipLine = pipOrFunction.call(this,filters);
        } else {
            pipLine = pipOrFunction;
        }
        ...
        var getCounts = function () {
            var count;
            collection.aggregate(pipLine.concat({$count:'count'})).forEach(x=>count=x.count);
            return count;
        }
        var getRows = function () {
            return _.map(
                collection.aggregate(pipLine.concat(
                [{'$sort':options.sort},{'$skip':options.skip},{'$limit':options.limit}]
            )), getRow
            );
        };
        var rows = {};
        ...
        self.added("reactive-table-counts", publicationId, {count: getCounts()});
        _.each(rows, function (row) {
            self.added("reactive-table-rows-" + publicationId, row._id, row);
        });

        var query = collection.find(pipOptions.observeSelector, pipOptions.observeOptions);
        if (!(settings || {}).disableRowReactivity) {
            var initializing = true;
            //var handle = pageCursor.observeChanges({
            var handle = query.observeChanges({
                added: function (id, fields) {
                    if (!initializing) {
                        updateRows();
                        self.changed("reactive-table-counts", publicationId, {count: getCounts()});
                    }
                },
